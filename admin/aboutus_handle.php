...
$(document).ready(function () {
	...
	$('#<?=$main_str?>_form').jqxValidator({
		rules: [
		{ input: '#content', message: '請輸入內容', action: 'keyup, blur', rule: 'required' }], 
		theme: theme
	});
});
</script>
   	 ...
   	 <div id="data_content">
   		<form method="post" id="<?=$main_str?>_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
                <tr>
                    <td height="60">About Us：</td>
                    <td><textarea id="content" name="content" rows="10"><?=stripslashes($contactus['content'])?></textarea></td>
                </tr>                
                <tr>
