<? 
	...
	$sqlC = "select * from waterfrom_proj2_category_titlecolor";
	$rlC = mysql_query($sqlC);
	
	$tmpC = mysql_num_rows($rlC);
	//echo $tmpC;
	if($tmpC > 0){
		$sql  = "Update waterfrom_proj2_category_titlecolor Set ";				
		$sql .= "color_code = '" . $color_code . "', ";												
		$sql .= "edit_time = '" . $tdt . "' ";
			
	}	else	{
		$sql  = "insert into waterfrom_proj2_category_titlecolor Values(";	
		$sql .= "'', ";							//Fullkey
		$sql .= "'', ";							//lang			
		$sql .= "'" . $color_code . "', ";		//color_code
		$sql .= "'1', ";						//pub				
...
<script>
	alert('資料已儲存。');
	location.replace('../proj2_category.php');
</script>