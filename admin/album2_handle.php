...
if($category!=0) {		
	$tmp_prev = $category;
	do{
		$query = "select id, name, prev, lv from waterfrom_proj2_category where id='".$tmp_prev."'"; 
		$pc = $obj_proj->run_mysql_out($query);
		$page_title = $pc['name'];
		$page_title_link = "album2.php?category=".$pc['id'];
		$page_subtitle = "<a href=proj2_category.php?category=".$pc['id']."><li class='left'>Press管理</li></a>".$page_subtitle;
		$tmp_prev = $pc['prev'];
	}while($pc['prev']!=0);
}
   	 ...
   	 <div id="data_content">
   		<form method="post" id="<?=$main_str?>_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
				...
				//echo '<tr><td colspan="3"><h2>專案圖片</h2>';
				for($i=3; $i<=3; $i++) { 
                	//$obj_image->uploadify_row('images', 1, $i, "專案圖片".$i, $main_str, $album['file'.$i], $album['title'], $_width_s, $_height_s, $_width, $_height, $file_size);
                } 
				echo '</td></tr>';
                ?>   
                ...
                ?>              
				<?
				echo '<tr><td colspan="3"><h2>上傳PDF</h2>';
				for($i=3; $i<=3; $i++) { 
                	$obj_image->uploadify_row('files', 1, $i, "上傳PDF".$i, $main_str, $album['file'.$i], $album['title'], $_width_s, $_height_s, $_width, $_height, $file_size);
                } 
				echo '</td></tr>';                
				?>
                ...
                <tr>
                    <td height="60">期數：</td>
                    <td align="left"><input type="text" name="period" id="period" value="<?=$album['period']?>" class="frome" style="width:400px" /></td>
                </tr> 
                <tr>
