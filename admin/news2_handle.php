...
$(document).ready(function () {
	...
	for($i=1; $i<=$images_num; $i++) { 
		$obj_image->uploadify_js('file'.$i.'_upload', 'file'.$i, 'show_file'.$i, array("action", "main_str", "category", "id", "file_o", "key", "_width_s", "_height_s", "need_thumb", "thumb_path"), array("upload_images", $main_str, $category, $news["id"], "", $i, $_width_s1, $_height_s1, "N", "thumb/"));
	} 
	?>
	var theme = '<?=jqxStyle?>';
   	 ...
   	 <div id="data_content">
   		<form method="post" id="<?=$main_str?>_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
				...
				for($i=1; $i<=$images_num; $i++) { 
                	$obj_image->uploadify_row('images', 2, $i, "上傳圖片".$i, $main_str, $news['file'.$i], $news['title'], $_width_s1, $_height_s1, $_width1, $_height1, $ImgFileSize1);
                } 
				echo '</td></tr>';
                ?>
                <tr>
