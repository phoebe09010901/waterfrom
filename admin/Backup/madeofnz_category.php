...
$query = "select id, content, lv, sort, pub, create_time, edit_time from $table_name where lang='".$_SESSION[Login_System_User]['lang']."' and prev=$prev $where_str order by sort desc";	
...
if($prev!=0) {		
	$tmp_prev = $prev;
	do{
		$query = "select id, content, prev, lv from $table_name where id='".$tmp_prev."'";
		$pc = $obj_cate1->run_mysql_out($query);
		$page_subtitle = "<a href=".$main_str.".php?prev=".$pc['prev']."><li class='left'>".$pc['content']."</li></a>".$page_subtitle;
		$tmp_prev = $pc['prev'];
	}while($pc['prev']!=0);
}
?><!DOCTYPE HTML>
