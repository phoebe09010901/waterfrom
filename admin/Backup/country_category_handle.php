...
$(document).ready(function () {
	...
	$('#<?=$main_str?>_category_form').jqxValidator({
		rules: [
		{ input: '#name', message: '請輸入國家', action: 'keyup, blur', rule: 'required' },
   	 ...
   	 <div id="data_content">
   		<form method="post" id="<?=$main_str?>_category_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
            	<?php if(country_category_Lv_Num > 1) { ?>
                <tr>
                    <td width="120" height="60">上層類別：</td>
                    <td align="left"><?php $sprods->data_category_select_reloop('country_category', 0, $prev, country_category_Lv_Num); ?></td>
                </tr>
                <?php } ?>
                <tr>
                    <td width="120" height="60">國　　家：</td>
